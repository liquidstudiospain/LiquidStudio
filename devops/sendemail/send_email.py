...
MR_IID = os.environ.get('gitlabMergeRequestIid', '125')
MR_TITLE = os.environ.get('gitlabMergeRequestTitle', 'This is a Mock Title')
TARGET_BRANCH = os.environ.get('gitlabTargetBranch', 'mock')
SOURCE_BRANCH = os.environ.get('gitlabSourceBranch', 'feature/mock')
# mr_Url = os.environ.get('CREDENTIALS_HOME')
MR_STATE = os.environ.get('gitlabMergeRequestState')
GITLAB_USERNAME = os.environ.get('gitlabUserName')
GITLAB_EMAIL = os.environ.get('gitlabUserEmail')
...
REMOTE_URL_ROUGH = os.environ.get('gitlabSourceRepoHttpUrl',
                                  'https://gitlab.com/mock/mock.git')
REMOTE_URL = get_remote_url(REMOTE_URL_ROUGH)
PWD = os.path.dirname(os.path.abspath(__file__))
ALLURE_URL = get_allure_url()
MR_URL = get_mr_url(REMOTE_URL_ROUGH, MR_IID)
...
def branch_error(smtp_server, recipients, recipients_cc, replyto=None):
    ''' Notifies invalid branch name '''
    print_key_value_list(f'{INFO_TAG} Sending \'selenium\' email:',
                         [('Recipients', recipients),
                          ('Recipients CC', recipients_cc),
                          ('Reply-To', replyto)])
    salutation = get_salutation()
    with open(f'{PWD}/resources/templates/errors/branches_error.html',
              encoding='utf8') as template:
        html_file = template.read()

    template = Template(html_file)
    html = template.safe_substitute(SALUTATION=salutation, MR_TITLE=MR_TITLE,
                                    TARGET_BRANCH=TARGET_BRANCH, MR_IID=MR_IID,
                                    MR_URL=MR_URL, BUILD_ID=BUILD_ID,
                                    SOURCE_BRANCH=SOURCE_BRANCH,
                                    JOB_LINK=BLUEOCEAN_URL)
    ...
    msg = MIMEMultipart()
    msg["Subject"] = f"[JENKINS][BRANCH-ERROR] #{MR_IID} - {MR_TITLE}"
    msg["From"] = smtp_server.user
    msg["To"] = ", ".join(recipients)
    msg["Cc"] = ", ".join(recipients_cc)
    msg.add_header('reply-to', replyto)
...
def __handle_options(smtp_server, args):
    ...
    elif args.option == 'branch_name':
        branch_error(smtp_server, args.recipients,
